...
.. note::
   - When :guilabel:`Booking` is enabled in the :ref:`POS settings <pos/use/settings>`, a
     ...
     and manage bookings.
   - Creating a restaurant from scratch creates a :guilabel:`Floor` in the :guilabel:`Floors &
     Tables Map` setting, under the :guilabel:`Point of Sale` section of the :ref:`POS settings
     <pos/use/settings>`. This floor is named after the database and is automatically assigned to
     the restaurant.
...
Plan`) icon in the top-right corner to open the :guilabel:`Floor plan editor`, then start
customizing using the relevant buttons:
...
.. tip::
   - There are two alternatives to create a floor plan. First, go to the :guilabel:`Point of Sale`
     section of the :ref:`POS settings <pos/use/settings>`. Under :guilabel:`Floors & Tables Map`,
     type the floor name in the :guilabel:`Floors` field, and press `Enter`. The second alternative
     is to go to :menuselection:`Point of Sale --> Configuration --> Floor Plans`, and click
     :guilabel:`New`. This action also allows the creation of tables.
...
#. Select :guilabel:`Preparation` in the :guilabel:`Type` field.
...
The printer is then connected to the point of sale and can print preparation orders.
