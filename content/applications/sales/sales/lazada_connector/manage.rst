...
<https://sellercenter.lazada.com.ph/>`, use the :guilabel:`Bulk Manage` drop-down to export your
product catalog, including Lazada SKUs.

   .. image:: manage/lazada-bulk-edit.png

2. Import into Odoo: Import the exported catalog into Odoo via :menuselection:`Inventory -->
   Products --> Import`. Map the Lazada SKU to the :guilabel:`Internal Reference` field in Odoo to
   link your Lazada and Odoo products.
...
.. important::
   Product catalog synchronization is an automated process initiated from the synchronization.
...
- Synchronization Scope: Only orders with status :guilabel:`READY_TO_SHIP` or :guilabel:`PROCESSED`
  are fetched, as these require shipping action.
...
1. Arrange Shipment: Confirm the picking in Odoo, then navigate to *Lazada Seller Center* and click
   :guilabel:`Pack Lazada Package` to generate the tracking number.
2. Fetch Shipping Label: Odoo retrieves the shipping label, attaching it to the corresponding
   delivery order.
3. Validate Stock Out: Validate the stock movement in Odoo to update inventory levels and confirm
...
- :guilabel:`Package Confirmed on Lazada`: The package has been packed by the seller or warehouse
  and is confirmed ready for courier pickup or dropoff. Lazada will be notified and update the
  order status.
- :guilabel:`Ready to Ship on Lazada`: The order is ready for shipment arrangement. Lazada will be
  notified and update the order status.
...
2. Arrange Shipment on Lazada: Click :guilabel:`Pack Lazada Package`` to arrange shipment in the
   delivery transfer if you are using supported logistics providers.
...
1. Issue Invoices: Generate invoices in Odoo and manually upload them to *Lazada Seller Center*.
2. Register Payments:

   - Create a dedicated :guilabel:`Bank Journal` (e.g., "Lazada Payments") with a Bank and Cash
     intermediary account.
   - Since Lazada processes weekly or monthly batch payments, select all invoices linked to a
     payment in Odoo.
...
3. Reconcile Payments: When Lazada deposits the balance, record it in the bank statement and credit
   the Lazada intermediary account.
...
2. Filter Sales Data: Use the dashboard filters to view sales data for the assigned Lazada sales
   team.
