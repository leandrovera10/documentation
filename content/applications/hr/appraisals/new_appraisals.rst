...
*Skills* tab of the appraisal form auto-populates with the skills from the :ref:`employee form
<employees/skills>`, once an appraisal is confirmed. Additionally, if the :guilabel:`Target Job`
field is populated, the corresponding skills and expected levels also appear in the *Skills* tab.
The target level for each skill appears in red, in the :guilabel:`Job Target` column.
...
enter any relevant details explaining the change, such as `took a fluency language test` or
`received Javascript certification`.
...
.. note::
   The *Skills* tab can be modified at anytime.
...
.. tip::
   ...
   :guilabel:`Reopen` button, then make any modifications needed. Once all modifications are
   complete, click the :guilabel:`Mark as Done` button again.
