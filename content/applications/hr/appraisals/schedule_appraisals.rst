...
The :guilabel:`Appraisals Plan` settings determine the frequency that appraisals are scheduled.
...
The :guilabel:`Appraisal Date` is when the appraisal is scheduled to be completed, and is scheduled
to be closed a month from the current day. Using the calendar selector, adjust the date, if desired.
This field is typically updated when the manager submits their final rating at the end of the
appraisal process.
...
Last, select the desired :guilabel:`Template`. The :guilabel:`Default` template populates this
field, by default, and is created when the **Appraisals** app is installed. Using the drop-down
